									...
									case "S":
										...
										var smtpConfig = {
											...
											auth: {
											  ...
											  pass: "gazu 5t3d 6hcd mxpe wydg xnjc fgd4 k44s" // "ktbcfacejdfpsjdy"
											},
											tls: {
...
nalazController.List = function(req, res) {
	...
	if (mongoose.connection.readyState != 1) {
		...
		if (req.query.datum === 'Svi Rezultati') {
			var datum14 = JSON.stringify(new Date(new Date().setDate(new Date().getDate() - 14))).substring(1, 11);
		...
		Results.find(uslov).populate('sample patient ').exec(function(err, results) {
			if (err) {
				console.log('Greška:', err);
			} else {
				switch (parametar) {
					...
					case 'ime':
						results = results.filter(function(result) {
							return result.patient.ime.toLowerCase().includes(req.query.filter.toLowerCase());
						});
						break;

					case 'prezime':
						results = results.filter(function(result) {
							return result.patient.prezime.toLowerCase().includes(req.query.filter.toLowerCase());
						});
						break;
					case 'jmbg':
						results = results.filter(function(result) {
							return result.patient.jmbg.toLowerCase().includes(req.query.filter.toLowerCase());
						});
						break;
					default:
						results = results.filter(function(result) {
							return (
								result.patient.ime.toLowerCase().includes(req.query.filter.toLowerCase()) ||
								result.patient.prezime.toLowerCase().includes(req.query.filter.toLowerCase())
							);
						});
						break;
				}
				switch (parametar) {
					...
					case 'ime':
						if (order === 'asc') {
							results.sort(function(a, b) {
								return a.patient.ime == b.patient.ime ? 0 : +(a.patient.ime > b.patient.ime) || -1;
							});
						}
						if (order === 'desc') {
							results.sort(function(a, b) {
					...
					case 'jmbg':
						if (order === 'asc') {
							results.sort(function(a, b) {
								return a.patient.jmbg == b.patient.jmbg ? 0 : +(a.patient.jmbg > b.patient.jmbg) || -1;
							});
						}
						if (order === 'desc') {
							results.sort(function(a, b) {
				...
				noviNiz.forEach((uzorak) => {
					...
					var datum = temp.getDate() + '-' + (temp.getMonth() + 1) + '-' + temp.getFullYear();
					...
					json.data.push({
						...
						// datum: datum
						datum:
							JSON.stringify(JSON.stringify(uzorak.created_at).substring(1, 11)).slice(9, 11) +
							'.' +
							JSON.stringify(JSON.stringify(uzorak.created_at).substring(1, 11)).slice(6, 8) +
							'.' +
							JSON.stringify(JSON.stringify(uzorak.created_at).substring(1, 11)).slice(1, 5)
					});
				});
				res.json(json);
			}
...
nalazController.Pregled = function(req, res) {
	if (mongoose.connection.readyState != 1) {
		...
		Results.find({
			...
			.populate('patient rezultati.labassay rezultati.rezultat.anaassay sample patient.lokacija sample.lokacija')
			.exec(function(err, results) {
				if (results.length) {
					...
					res.json((success = true), (message = 'Nema pronadjenih rezultata'), (results = []));
				}
			});
	}
};
...
nalazController.Nalaz = function(req, res) {
	if (mongoose.connection.readyState != 1) {
		...
		Results.find(uslov)
			.populate('patient rezultati.labassay rezultati.rezultat.anaassay sample patient.lokacija site')
			.exec(function(err, results) {
				if (results.length) {
					var uzorci = [];
					var rezultati = [];
						...
						req.body.samples.forEach((sample) => {
							if (mongoose.Types.ObjectId(sample).equals(element._id)) {
								...
								element.rezultati.forEach((rezultat) => {
									checkIskljucen = iskljucen.test(rezultat.labassay.naziv, req.body.site);
									if (
										...
										element.multi.forEach((multi) => {
											multi.forEach((mm) => {
												if (rezultat.labassay._id.equals(mongoose.Types.ObjectId(mm.labtest)))
													if (
														mm.rezultat[0].rezultat_f != '' &&
														!mm.rezultat[0].rezultat_f.includes('--') &&
														iskljucen.test(mm.rezultat[0].dilucija, req.body.site)
													) {
                              ...
                              test.rezultat = mm.rezultat[0].rezultat_f;
											...
											if (ana.length) {
												if (!element.site.postavke.nalazLegenda) {
													...
													if (
														!legenda.filter((tip) => tip === element.id.substring(0, 1))
															.length > 0
													) {
										...
										if (
											...
											checkIskljucen &&
											rezultat.rezultat[rezultat.rezultat.length - 1].rezultat_f != '' &&
											...
											if (
												...
												rezultat.rezultat[rezultat.rezultat.length - 1].rezultat_m.length > 0
											) {
												mikrobiologija = true;
											...
											if (element.site.postavke.nalazLegenda) {
												...
												test.naziv =
													'[' + element.id.substring(0, 1) + '] ' + rezultat.labassay.analit;
											...
											test.jedinica = rezultat.rezultat[rezultat.rezultat.length - 1].jedinice_f;
											test.rezultat = rezultat.rezultat[rezultat.rezultat.length - 1].rezultat_f;
											...
											test.anaassay = rezultat.rezultat[rezultat.rezultat.length - 1].anaassay;
											...
											if (
												rezultat.rezultat[rezultat.rezultat.length - 1].anaassay.float != '' &&
												!isNaN(
													rezultat.rezultat[rezultat.rezultat.length - 1].anaassay.float
												) &&
												!isNaN(test.rezultat)
												...
												test.rezultat = parseFloat(test.rezultat).toFixed(float);
											}
											...
											if (!mikrobiologija) {
												novirezultati.push({
													...
													data: rezultat.rezultat[rezultat.rezultat.length - 1].rezultat_m
												});
											} else {
												var kontrola = 'No Class';
												if (
													rezultat.rezultat[rezultat.rezultat.length - 1].rezultat_m[0]
														.pozitivan
												) {
													kontrola = 'Red';
												} else if (
					...
					novirezultati.forEach((element) => {
						i++;
						if (tempniz.filter((e) => e.grupa === element.grupa).length > 0 || !tempniz.length) {
					...
					novirezultati.forEach((element) => {
						...
						if (!element.mikrobiologija) {
							if (tempniz.filter((e) => e.sekcija === element.sekcija).length > 0 || !tempniz.length) {
								tempniz.push(element);
								if (i === novirezultati.length) {
		...
		Nalazi.findOne({
			...
			if (err) {
				...
				if (!nalaz) {
					...
					nalaz.remove();
					fs.unlink(config.nalaz_path + req.body.idNalaza + '.pdf', function(err) {
						if (err) {
							...
							if (req.body.flag) {
								...
								var vrijeme = d.getHours() + ':' + d.getMinutes() + ':' + d.getSeconds();
...
nalazController.outboxNalaz = function(req, res) {
	...
	if (mongoose.connection.readyState != 1) {
		...
		Outbox.findOne(uslov).populate('patient nalaz').exec(function(err, nalaz) {
			if (err) {
				res.json({
					success: false,
					message: 'Greška!'
...
nalazController.checkifComplete = function(req, res) {
	...
	if (mongoose.connection.readyState != 1) {
		...
		Results.find(uslov).populate('patient sample').exec(function(err, rezultati) {
			if (err) {
				res.json({
					success: false,
					message: 'Greška!'
